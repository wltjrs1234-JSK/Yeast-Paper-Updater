...
이 프로젝트는 매월 1일 자동으로 S. cerevisiae(효모)와 관련된 지난 30일(1개월)간의 최신 연구 문헌(논문, 프리프린트 등)을 검색하여 이메일로 전송해주는 파이프라인입니다.
...
## 검색 카테고리
1. **Glutathione & GSH (생산 및 개량)**: 효모의 대사 공학, 균주 개량, 발효 등을 통한 글루타티온(GSH) 생산성 향상 관련 논문
2. **NAD+ & Precursors (생산 및 개량)**: 효모를 이용한 NMN, NR, NAD, NAM 등의 물질 대사 공학 및 생산성 향상 관련 논문
3. **Gene Editing (새로운 툴 및 기술)**: 효모에 적용할 수 있는 새롭게 개발된 유전자 조작 툴(CRISPR, 플랫폼 등) 또는 획기적으로 개선된 방법론 관련 논문
...
- `EMAIL_RECEIVER`: 메일을 수신받을 이메일 주소. (여러 명에게 동시에 보내려면 쉼표(`,`)로 구분해서 적어주세요. 예: `myid@naver.com, other@gmail.com, third@daum.net`)

### 3. 수동 테스트
`Actions` 탭으로 이동하여 `S. cerevisiae Literature Monthly Update` 워크플로우를 선택한 뒤, **Run workflow** 버튼을 눌러 스크립트가 정상적으로 실행되고 이메일이 수신되는지 확인합니다.

---
성공적으로 설정되었다면, 매월 1일 오전 9시(한국시간 기준)에 지난 30일 동안 새롭게 등록된 문헌들이 요약본(Abstract) 및 링크와 함께 이메일로 자동 전송됩니다.
